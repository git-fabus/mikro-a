...
\begin{solution}
    Wir optimieren
    \[
        u(C,F) = C \cdot (28-L)^2 \quad \text{und NB } C= L^{\frac{1}{3}}
    .\] 
    Bzw. $pC=wL$.
    ...
    Also erhalten wir:
    \[
    u(L) = \left( \frac{w}{p} L  \right) \cdot (28-L)^2
    .\] 
    Die FOC ergibt sich als
    \[
    u'(L) = \frac{w}{p} (28-L) \cdot (28-3L)
    .\] 
    Damit ergibt sich für $L = \frac{28}{3}$ oder $L=28$, aber da bei $L=28$ keine Freizeit bleibt, wäre der Nutzen $u=0$, somit ist $L^* = \frac{28}{3}$ und
    \[
        \frac{w}{p} \frac{28}{3} = L^{\frac{1}{3}} = \left( \frac{28}{3} \right)^{\frac{1}{3}}
    .\]
    Robinson kann also mit einem Stundenlohn $\frac{28}{3}$ Kokusnüsse kaufen.  
\end{solution}
